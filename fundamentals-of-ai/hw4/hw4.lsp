...
;
(defun at-most-helper (n c l k)
	(if (> l k) 
		nil
		(cons (list (- (node2var n c k)) (- (node2var n l k))) (at-most-helper n c (+ l 1) k))
	)
)
...
(defun at-most-one-color (n c k)
	(if (= c k)
		nil
		(append (at-most-helper n c 1 k) (at-most-one-color n (+ c 1) k))
	)
)
...
(defun generate-edge-clauses-helper (x y c k)
	(if (> c k)
		nil
		(cons (list (- (node2var x c k)) (- (node2var y c k))) (generate-edge-clauses-helper x y (+ c 1) k))
	)
)

(defun generate-edge-clauses (e k)
  (generate-edge-clauses-helper (car e) (car (cdr e)) 1 k)
)
