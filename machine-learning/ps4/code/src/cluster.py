...
class Cluster(object) :
    ...
    def purity(self) :
        ...
        return len(labels), np.float64(count), cluster_label
    ...
    def centroid(self) :
        ...
        attrs = np.array([point.attrs for point in self.points])
        mean_attrs = np.mean(attrs, axis=0)
        _, _, most_common_label = self.purity()

        centroid = Point('centroid', str(most_common_label), mean_attrs)
        return centroid
        ### ========== TODO : END ========== ###
    ...
    def medoid(self) :
        ...
        medoid = self.points[np.argmin(distances)]
        return medoid
        ### ========== TODO : END ========== ###
